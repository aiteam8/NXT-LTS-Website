...
document.querySelectorAll('.lang-btn[data-lang]').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    const inAr       = path.includes('/ar/');
    const inSvc      = path.includes('/services/');

    if (targetLang === 'ar' && !inAr) {
      // EN → AR
      window.location.href = inSvc
        ? '../ar/services/' + file
        : 'ar/' + file;
    } else if (targetLang === 'en' && inAr) {
      // AR → EN
      window.location.href = inSvc
        ? '../../services/' + file
        : '../' + file;
...
document.querySelectorAll('.lang-btn[onclick]').forEach(() => {});
...
document.querySelectorAll('.svc-card, .card').forEach((el, i) => {
  if (!el.closest('.services-carousel')) {
    el.style.opacity = '0';
...
(function buildCarousel() {
  ...
  // Only on index/home pages
  const path   = window.location.pathname;
  const isHome = path.endsWith('/') || path.endsWith('index.html') || path === '' || /\/ar\/?(index\.html)?$/.test(path);
  if (!isHome) return;
  ...
  allCards.forEach(card => {
    // Reset any stagger styles applied above
